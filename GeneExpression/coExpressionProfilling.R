...
write.csv(vsd, "outputs/variance.Stablised.Data.csv")
rv_vsd = rowVars(vsd)
q75 <- quantile( rowVars(vsd), .75)
q95 <- quantile( rowVars(vsd), .95)
normalised.d = vsd[rv_vsd>q95, ]
write.csv(normalised.d, "outputs/normalised.Heat.Counts.csv")
...
df = read.delim("~/Documents/hogprof/hsps.tsv")
library(openxlsx)
write.xlsx(df, "~/Documents/hogprof/hsps.xlsx", row.names =F)
install.packages("Displayr")
df = read.csv("inputs/profile.csv", na.strings=c("","NA"))
summary(df)
colnames(df)
...
load("/Users/zitu/Downloads/Marker_matrices.RData")
