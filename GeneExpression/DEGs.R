...
design.logFC = data.frame(tissue, stress, genotype)
...
for (i in 2:length(col)) {
  ...
  if(genotype == "") {
    ...
    design.rest = data.frame(tissue, stress, genotype)
...
pdf(ht, "outputs/figures/heatmap.all.pdf")
...
pdf(h, "outputs/figures/heatmap.all.selected.pdf")
#upset plot for DEGs
...
pdf(u, "outputs/figures/upregulated.all.pdf")
...
pdf(d, "outputs/figures/downregulated.all.pdf")

library(dplyr)
row.names(df) = df$Solyc00g160260.1.1.Homeobox.leucine.zipper.protein..AHRD.V3.11.....tr.Q8H963.Q8H963_ZINVI.
df = df[c(grep("Heat", row.names(df))),]
df = data.frame(df)
library(openxlsx)
write.xlsx(df, "~/Documents/transcriptomic/ITAG4.1/hs.xlsx")


library(foreign)
data <- read.spss("/Users/zitu/Downloads/adaptation of farming practices by the coastal farmers in response to climate change (8) (1) (1).sav", to.data.frame=TRUE)
colnames(data)
